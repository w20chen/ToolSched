...
schema, then evaluates prediction models across four distinct problems.
...
5. [Problem 4: Agent Remaining Time Prediction](#5-problem-4-agent-remaining-time-prediction)
6. [Features: What Models Can Learn From](#6-features-what-models-can-learn-from)
7. [Calibration: Online Correction at Inference Time](#7-calibration-online-correction-at-inference-time)
8. [Evaluation: How Models Are Scored](#8-evaluation-how-models-are-scored)
...
3. **Build history**: A sliding window of the last `k` tool names is tracked
   as the `history` field.
4. **Set `next_tool`**: The tool name of the *next* call in the array becomes
...
properties, and recent history tools, all encoded as
...
**Prediction**: `predict_index(sample)` returns the bucket index (0â€?).
...
| **Long Task Recall (â‰?0s)** | Recall on buckets 3 and 4 combined (tools taking â‰?0 seconds) |
...
grouped by `(tool_family, operation)`, including:
...
## 5. Problem 4: Agent Remaining Time Prediction
...
| 1 | 15â€?0 s | Short â€?quick scheduling |
| 2 | 60â€?80 s | Moderate â€?normal scheduling |
| 3 | 180â€?00 s | Long â€?significant resource commitment |
| 4 | 600â€?800 s | Heavy â€?consider preemption |
...
1. **Predicts remaining steps** (not time) via a softmax classifier over
   step-count buckets (â‰?, 4â€?0, 11â€?5, 26â€?0, >50)
...
## 6. Features: What Models Can Learn From
...
## 7. Calibration: Online Correction at Inference Time
...
## 8. Evaluation: How Models Are Scored
...
| **Pinball Loss** | A quantile-specific loss function: `max(q Ã— (yâˆ’Å?, (qâˆ?) Ã— (yâˆ’Å?)`. Asymmetric: penalizes under-prediction more when q > 0.5. |
